...
# A grammar for combining data ('join' logic) ------------------------------------------
  ...
  # Practice! -----
        ...
        dd = left_join(chgdppc,repress,by=c("country_name","year"))
        dd = left_join(dd,infmort,by=c("country_name","year"))

        dd %>% arrange(desc(year))

        dd %>% filter(is.na(infmort)) %>% group_by(country_name) %>% tally()
...
# Applied Examples:
      ...
      # How many events of each event type happened by year in Nigeria?
          ...
          p1 = ggplot(dat,aes(x=year)) +
            geom_bar() +
            facet_wrap(~event_type,ncol=3,scales = "free") +
